...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^RRP hastle (.+) (\d+[smhd])$'))
    ...
    @client.on(events.NewMessage(pattern=r'^RRP stop$'))
    ...
    # Tes koneksi bot
    @client.on(events.NewMessage(pattern=r'^RRP ping$'))
    async def ping_handler(event):
        await event.reply("🏓 Pong! Bot aktif.")

    # Broadcast pesan ke semua chat kecuali blacklist
    @client.on(events.NewMessage(pattern=r'^RRP bcstar (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^RRP bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^RRP stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^RRP bl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^RRP unbl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^RRP help$'))
    async def help_handler(event):
        help_text = (
            "📋 **Daftar Perintah yang Tersedia:**\n\n"
            "1. RRP hastle [pesan] [waktu][s/m/h/d]\n"
            ...
            "5. RRP bcstargr [waktu][s/m/h/d] [pesan]\n"
            "   Broadcast hanya ke grup dengan interval tertentu.\n"
            "6. RRP stopbcstargr[1-10]\n"
    ...
    @client.on(events.NewMessage(pattern=r'^RRP setreply'))
    ...
    @client.on(events.NewMessage(pattern=r'^RRP stopall$'))
